...
function TabsList({ className, ...props }: TabsPrimitive.TabsListProps) {
  return (
    <TabsPrimitive.List
      className={cn(
        "flex items-center overflow-x-auto border-b border-neutral-200 w-full",
        // Hide scrollbar visually while keeping scroll functionality
        "[&::-webkit-scrollbar]:h-0 [&::-webkit-scrollbar]:hidden",
        className
      )}
      {...props}
    />
  ...
  return (
    <TabsPrimitive.Trigger
      className={cn(
        ...
        "text-xs sm:text-sm font-medium text-neutral-500 transition-all",
